...
uint8_t UltrasonicWave_Distance;      //计算出的距离    
...
u8 Get_Distance(){
	...
	delay_us(10);		                      //延时10us
	while(GPIO_ReadInputDataBit(ECHO_PORT,ECHO_PIN)==0);		//等待ECHO高电平信号
			...
			UltrasonicWave_Distance=TIM_GetCounter(TIM2)*5*34/100;									 //计算距离&&UltrasonicWave_Distance<150
...
void Distance_tx_lora(u8 distance){
	u8 t[100],tx[100]="jiaoyan:distance ";
